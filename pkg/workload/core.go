	...
	for i, freq := range c.keyRangeFreqs {
		if freq > 0 {
			...
			if freqRatio >= 0.05 || valueSizeRatio >= 0.05 {
				fmt.Println("freq ratio (%) ", freqRatio, ", valueSize ratio (%)", valueSizeRatio, "; avg valueSize", float64(c.keyRangeSumVSize[i])/float64(freq))
			}
		}
	}
	return nil
...
func (c *core) buildKeyName(state *coreState, keyNum int64) string {
	if !c.orderedInserts {
		keyNum = util.Hash64(keyNum)
	}

	prefix := c.p.GetString(prop.KeyPrefix, prop.KeyPrefixDefault)
	return fmt.Sprintf("%s%03dt%0[4]*[3]d", prefix, state.curTable, keyNum, c.zeroPadding)
}
...
func (c *core) buildDeterministicValue(state *coreState, key string, fieldKey string) []byte {
	...
	buf := c.getValueBuffer(int(size + 25))
...
func (c *core) DoInsert(ctx context.Context, db ycsb.DB) error {
	...
	r := state.r
	state.curTable = c.tableChooser.Next(r) % c.tableNum
	keyNum := c.keySequence[state.curTable].Next(r)
	state.curValueSize = ParetoCdfInversion(r.Float64(), c.valueTheta, c.valueK, float64(c.avgValueSizes[state.curTable]))
	...
	dbKey := c.buildKeyName(state, keyNum)
...
func (c *core) InitMixGraphBench() {
	...
	case "sequential":
		c.tableChooser = generator.NewSequential(0, c.tableNum-1)
	case "zipfian":
		c.tableChooser = generator.NewScrambledZipfian(0, c.tableNum-1, generator.ZipfianConstant)
	case "hotspot":
		hotsetFraction := c.p.GetFloat64(prop.HotspotDataFraction, prop.HotspotDataFractionDefault)
		hotopnFraction := c.p.GetFloat64(prop.HotspotOpnFraction, prop.HotspotOpnFractionDefault)
		c.tableChooser = generator.NewHotspot(0, c.tableNum-1, hotsetFraction, hotopnFraction)
	case "exponential":
		percentile := c.p.GetFloat64(prop.ExponentialPercentile, prop.ExponentialPercentileDefault)
		frac := c.p.GetFloat64(prop.ExponentialFrac, prop.ExponentialFracDefault)
		c.tableChooser = generator.NewExponential(percentile, float64(c.tableNum)*frac)
	default:
		util.Fatalf("unknown request distribution %s", tableDistrib)
	}
	...
	c.keyRangeSize = c.recordCount * c.tableNum / (rangeNum - 1)
...
func (c *core) MixGraphBench(ctx context.Context, db ycsb.DB) error {
	...
	keyName := c.buildKeyName(state, keyRand)
	queryType := c.queryChooser.Next(r) //or generated by randV

	c.keyRangeFreqs[(keyRand+c.recordCount*state.curTable)/c.keyRangeSize]++
	...
	if queryType == 0 {
		...
		state.curValueSize = ParetoCdfInversion(u, c.valueTheta, c.valueK, float64(c.avgValueSizes[state.curTable]))
		if state.curValueSize <= 0 {
			state.curValueSize = 10
		} else if state.curValueSize > c.fieldLength {
			state.curValueSize = state.curValueSize % c.fieldLength
		...
		c.keyRangeSumVSize[(keyRand+c.recordCount*state.curTable)/c.keyRangeSize] += state.curValueSize
...
func (c *core) nextKeyNum(state *coreState) int64 {
	r := state.r
	keyNum := int64(0)
	if _, ok := c.keyChooser[state.curTable].(*generator.Exponential); ok {
		keyNum = -1
		for keyNum < 0 {
			keyNum = c.transactionInsertKeySequence[state.curTable].Last() - c.keyChooser[state.curTable].Next(r)
		}
	} else {
		keyNum = math.MaxInt64
		for keyNum > c.transactionInsertKeySequence[state.curTable].Last() {
			keyNum = c.keyChooser[state.curTable].Next(r)
		}
	}
	return keyNum
}
...
func (c *core) doTransactionRead(ctx context.Context, db ycsb.DB, state *coreState) error {
	...
	keyName := c.buildKeyName(state, keyNum)
...
func (c *core) doTransactionInsert(ctx context.Context, db ycsb.DB, state *coreState) error {
	r := state.r
	keyNum := c.transactionInsertKeySequence[state.curTable].Next(r)
	defer c.transactionInsertKeySequence[state.curTable].Acknowledge(keyNum)
	dbKey := c.buildKeyName(state, keyNum)
	values := c.buildValues(state, dbKey)
	defer c.putValues(values)
...
func (c *core) doTransactionScan(ctx context.Context, db ycsb.DB, state *coreState) error {
	r := state.r
	keyNum := c.nextKeyNum(state)
	startKeyName := c.buildKeyName(state, keyNum)
...
func (c *core) doBatchTransactionRead(ctx context.Context, batchSize int, db ycsb.BatchDB, state *coreState) error {
	...
	for i := 0; i < batchSize; i++ {
		keys[i] = c.buildKeyName(state, c.nextKeyNum(state))
	}
...
func (c *core) doBatchTransactionInsert(ctx context.Context, batchSize int, db ycsb.BatchDB, state *coreState) error {
	...
	for i := 0; i < batchSize; i++ {
		...
		c.transactionInsertKeySequence[state.curTable].Acknowledge(keyNum)
	}
...
func (coreCreator) Create(p *properties.Properties) (ycsb.Workload, error) {
	...
	tableNum := c.p.GetInt64(prop.TableNum, prop.TableNumDefault)
	c.tableNum = tableNum
	c.recordCount = p.GetInt64(prop.RecordCount, prop.RecordCountDefault) / tableNum
	if c.recordCount == 0 {
	...
	insertStart := p.GetInt64(prop.InsertStart, prop.InsertStartDefault) / tableNum
	insertCount := c.recordCount - insertStart
	if _, ok := p.Get(prop.InsertCount); ok {
		insertCount = p.GetInt64(prop.InsertCount, 0) / tableNum
	}
	if c.recordCount < insertStart+insertCount {
		util.Fatalf("record count %d must be bigger than insert start %d + count %d",
			c.recordCount*tableNum, insertStart*tableNum, insertCount*tableNum)
	}
	...
	for i := int64(0); i < tableNum; i++ {
		c.keySequence = append(c.keySequence, generator.NewCounter(insertStart/tableNum))
	}
	c.operationChooser = createOperationGenerator(p)

	for i := int64(0); i < tableNum; i++ {
		c.transactionInsertKeySequence = append(c.transactionInsertKeySequence, generator.NewAcknowledgedCounter(c.recordCount))
		var keyChooser ycsb.Generator
		switch requestDistrib {
		case "uniform":
			keyChooser = generator.NewUniform(insertStart, insertStart+insertCount-1)
		case "sequential":
			keyChooser = generator.NewSequential(insertStart, insertStart+insertCount-1)
		case "zipfian":
			insertProportion := p.GetFloat64(prop.InsertProportion, prop.InsertProportionDefault)
			opCount := p.GetInt64(prop.OperationCount, 0) / tableNum
			expectedNewKeys := int64(float64(opCount) * insertProportion * 2.0)
			keyChooser = generator.NewScrambledZipfian(insertStart, insertStart+insertCount+expectedNewKeys, generator.ZipfianConstant)
		case "latest":
			keyChooser = generator.NewSkewedLatest(c.transactionInsertKeySequence[i])
		case "hotspot":
			...
			keyChooser = generator.NewHotspot(insertStart, insertStart+insertCount-1, hotsetFraction, hotopnFraction)
		case "exponential":
			...
			keyChooser = generator.NewExponential(percentile, float64(c.recordCount)*frac)
		default:
			util.Fatalf("unknown request distribution %s", requestDistrib)
		}
		c.keyChooser = append(c.keyChooser, keyChooser)
	}
